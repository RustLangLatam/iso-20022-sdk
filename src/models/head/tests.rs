...
mod tests {
    ...
    fn test_parse_xml_head_001_001_01() {
        let file = std::fs::read_to_string("test/resources/head/head.001.001.01-sgntr-ecdsa.xml").expect("Unable to read file");

        let doc = AppHdr::<EcdsaSignature>::from_xml(file.as_str());

        assert!(doc.is_ok_and(|v| v.is_head_001_001_01()));
    }
    ...
    fn test_parse_xml_head_001_001_02() {
        ...
        assert!(doc.is_ok_and(|v| v.is_head_001_001_02()));
    }
}
